...
fig.canvas.mpl_connect("button_press_event", lambda event: on_hover_intensity_merged(event, file_path, fig, ax, scatter_data, image_copy, image, mask = (circle_array_new == [255, 255, 0]).all(axis=-1), tpf=1/50))
fig.canvas.mpl_connect("motion_notify_event", lambda event: on_hover_intensity_merged(event, file_path, fig, ax, scatter_data, image_copy, image,mask = (circle_array_new == [255, 255, 0]).all(axis=-1), tpf=1/50))
