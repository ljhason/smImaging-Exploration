...
fig.canvas.mpl_connect("button_press_event", lambda event: on_hover_intensity_merged(event, file_path, fig, ax, scatter_data, y_centres, x_centres, image_copy, image, mask = (circle_array_new == [255, 255, 0]).all(axis=-1), radius=4, tpf=1/50, CH_consideration=True))
fig.canvas.mpl_connect("motion_notify_event", lambda event: on_hover_intensity_merged(event, file_path, fig, ax, scatter_data, y_centres, x_centres, image_copy, image,  mask = (circle_array_new == [255, 255, 0]).all(axis=-1), radius=4, tpf=1/50, CH_consideration=True))
