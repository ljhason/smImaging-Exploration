    ...
    for scatter, peaks, label in scatter_data:
        ...
        if cont:
            ...
            if event.name == "button_press_event":
                ...
                rect1 = patches.Rectangle((x1_CH1, y1_CH1), x2_CH1 - x1_CH1, y2_CH1 - y1_CH1, linewidth=2, edgecolor='g', facecolor='none')
                ax.add_patch(rect1)
...
fig.canvas.mpl_connect("button_press_event", lambda event: on_hover_intensity(event, file_path, fig, ax, scatter_data, image_copy, image))
fig.canvas.mpl_connect("motion_notify_event", lambda event: on_hover_intensity(event, file_path, fig, ax, scatter_data, image_copy, image))
plt.grid()
plt.show()